...
(function () {
  ...
  function injectCSS() {
    ...
    s.textContent = [
      ...
      /* ── HEADER — fixed height always, two opacity layers cross-fade ──────
       * #cat-hdr          : sticky, fixed 156px — NEVER changes height
       * #cat-hdr-banner   : blurred image background, absolute, always present
       * #cat-hdr-scrim    : darkening gradient over banner
       * #cat-hdr-expanded : big avatar + name + meta — opacity:1 at rest
       * #cat-hdr-compact  : small avatar + name inline — opacity:0 at rest
       * JS toggles .scrolled on #cat-hdr → swaps opacity only.
       * No height change. No reflow. No feedback loop. ── */

      '#cat-hdr{position:sticky;top:0;z-index:100;',
        'height:156px;', /* fixed — never changes */
        'overflow:hidden;',
        'border-bottom:1px solid rgba(255,255,255,0.08);}',

      /* Banner — blurred image, always fills header */
      '#cat-hdr-banner{',
      ...
      /* Large avatar */
      '#cat-avatar{width:52px;height:52px;border-radius:14px;flex-shrink:0;',
        'background:rgba(255,255,255,0.08);',
        'border:1.5px solid rgba(255,255,255,0.2);',
        'box-shadow:0 4px 16px rgba(0,0,0,0.45);',
        'display:flex;align-items:center;justify-content:center;',
        'font-size:18px;font-weight:900;color:#fff;overflow:hidden;}',
      ...
      /* Tagline — shown only in expanded, hidden when no tagline */
      ...
      /* compact status pill — matches expanded styling */
  ...
  function buildShell() {
    ...
    // Layer 2: expanded — big avatar, centred name, meta row
    ...
    // Layer 3: compact — small avatar left, name, status pill
    ...
    // Status pill lives in compact bar (same id — only one is visible at a time)
  ...
  function attachEvents(storeName) {
    ...
    /* ── Header cross-fade on scroll ────────────────────────────────────────
     * Toggles .scrolled on #cat-hdr — CSS handles all visual change.
     * Threshold = header's own offsetHeight so the crossfade triggers
     * exactly when the user has scrolled one full header height.
     * rAF gate + passive listener = zero jank on mid-range Android. ── */
    var _hdrEl = document.getElementById('cat-hdr');
    if (_hdrEl) {
      var _scrolled    = false;
      var _rafPending  = false;
      var _THRESHOLD   = _hdrEl.offsetHeight || 156;
      ...
      function _onScroll() {
        ...
        requestAnimationFrame(function () {
          _rafPending = false;
          var past = (window.scrollY || window.pageYOffset) > _THRESHOLD;
          if (past === _scrolled) return;
          _scrolled = past;
          _hdrEl.classList.toggle('scrolled', past);
        });
      }
